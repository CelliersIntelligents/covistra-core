...
function CMBF() {
    this.hooks = {};
    this.log = bunyan.createLogger({name: 'core', level: 'trace'});

    if(process.env.NODE_ENV !== 'production') {
        this.log.warn("Enabling Bluebird long stack trace in development and test mode");
        P.longStackTraces();
    }
}
...
CMBF.prototype.callHook = function(key, params, defaultImpl) {
    ...
    if(hook) {
        ...
        return hook.call(this)(params, defaultImpl);
    }
    else if(defaultImpl) {
...
CMBF.prototype.launch = P.method(function() {
    var _this = this;
    ...
    return this.callHook('configure-server', serverOpts).then(function(opts) {
        ...
        return _this.callHook('register-early-plugins', {server:server}, function() {
            ...
            return P.join(
                P.promisify(server.register, server)({register: require('hapi-config')})
            );
        ...
        .then(function() {
            ...
            return _this.callHook('register-connections', {server:server, config:config}, function() {
                ...
                var apiOpts = {
                    port: config.get('PORT'), labels: 'api',
                    router: {
                ...
                var adminOpts = {
                    port: config.get('ADMIN_PORT'), labels: 'admin',
                    router: {
                        stripTrailingSlash: true
                    },
                    routes: {
                ...
                return _this.callHook('configure-connections', {config:config, server:server, connections:[apiOpts, adminOpts]}).then(function(result) {
            ...
            .then(function() {
                ...
                return _this.callHook('configure-security', {server:server, config:config}, function() {
                    ...
                    return P.join(
                        P.promisify(server.register, server)({register: require('hapi-auth-bearer-token')}),
                        P.promisify(server.register,server)({register: require('hapi-auth-cookie')}),
                        P.promisify(server.register, server)({register: require('hapi-auth-basic')})
                    );
            ...
            .then(function() {
                ...
                return _this.callHook('configure-documentation', {server:server, config:config, swaggerOpts:swaggerOpts}).then(function(result) {
                    ...
                    return P.join(
                        P.promisify(server.register, server)({register: require('inert')}),
                        P.promisify(server.register, server)({register: require('vision')}),
            ...
            .then(function() {
                ...
                return _this.callHook('register-plugins', {server:server, config:config}, function() {
                    _this.log.debug("Loading default foundation plugins: hapi-bunyan, covistra-system, covistra-socket, covistra-messaging");
                    ...
                    return P.join(
                        ...
                        P.promisify(server.register, server)({register: require('covistra-system')}),
                        P.promisify(server.register, server)({register: require('../../covistra-security')}),
                        P.promisify(server.register, server)({register: require('covistra-socket')}),
                        P.promisify(server.register, server)({register: require('covistra-messaging')})
                    );
                });
            })
        ...
        // Launch is complete
        .then(function() {
            _this.log.debug("Server was successfully started");
            _this.server = server;
            return _this;
        });
