...
CMBF.prototype.registerHooks = function(hooks) {
    var _this = this;
    return P.each(_.keys(hooks), function(hookKey) {
...
CMBF.prototype.installPlugin = P.method(function(plugin, options) {
    ...
    return this.waitFor(plugin.deps || options.deps || [], {debug: entry.name, timeout: 30000}).then(function() {
        _this.log.trace("Plugin %s dependencies are all resolved", path);
        return P.promisify(_this.server.register, _this.server)({register: plugin, options:options.runtime}).then(function() {
...
CMBF.prototype.launch = P.method(function(pluginOpts) {
    ...
    return _this.callHook('inject-dependencies', { injector: this.injector})
                ...
                .then(function() {
                    ...
                    return _this.callHook('register-plugins', {server:server, config:config}, function() {
                        ...
                        return P.join(
                            ...
                            _this.installPlugin(require('covistra-system'), {runtime: pluginOpts}),
                            _this.installPlugin(require('covistra-mongodb'), {runtime: pluginOpts}),
                            _this.installPlugin(require('covistra-security'), {runtime: pluginOpts}),
                            _this.installPlugin(require('covistra-admin'), {runtime: pluginOpts})
                        ).then(function() {
                           _this.log.info("All default plugins were successfully registered");
                        });
                    });
                })
